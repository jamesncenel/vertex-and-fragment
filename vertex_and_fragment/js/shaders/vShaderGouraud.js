...
void main() {
	...
	for(int i = 0; i < NUM_POINT_LIGHTS; i++)
	{
		...
		// perfectReflector = -1.0 * reflect(lightVector, vec4(normInViewSpace, 0.0));
		perfectReflector = 2.0*(dot(lightVector, vec4(normInViewSpace, 0.0))) * vec4(normInViewSpace, 0.0) - lightVector;
		specularIntensity = pow(max(dot(perfectReflector, vec4(normInViewSpace, 0.0)), 0.0), material.shininess);
		...
		vColor += (falloff * (diffuseReflection + specularReflection));
	}
	// vColor = ambientReflection + (falloff * (diffuseReflection + specularReflection)) ;
